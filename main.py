...
for i in range(5):
    try:
        pool = mysql.connector.pooling.MySQLConnectionPool(
            pool_name="mypool",
            pool_size=5,
            host=DB_URL,  #"34.44.150.229", #127.0.0.1",                       #34.44.150.229",
            user="zan",
    ...
    try:
        with pool.get_connection() as conn:
            with conn.cursor() as cursor:
                ...
                sql = "SELECT IDPonudba, IDPoslovalnica, IDStoritev, Aktiven FROM " + tennantDB + ".Ponuja WHERE IDPonudba = %s"
...
def posodobi_ponudbo(ponu: Ponudba1):
    ...
    try:
        ...
        if ponu.aktiven != '1':
            ponu.aktiven = '0'
        query = "SELECT IDPonudba, IDPoslovalnica, IDStoritev, Aktiven FROM  " + tennantDB + ".Ponuja WHERE IDPoslovalnica = %s AND IDStoritev = %s AND IDPonudba != %s LIMIT 1"
        cursor.execute(query,(ponu.idposlovalnica,ponu.idstoritev,ponu.idponudba))
        row = cursor.fetchone()
        print(row)
        if row is None:
    ...
    try:
        with pool.get_connection() as conn:
            with conn.cursor() as cursor:
                ...
                sql = "SELECT z.IDZaposleni, z.Ime, z.Priimek, z.Telefon, z.Email, p.NazivPoslovalnice FROM "+ tennantDB +".Zaposleni z, "+tennantDB+".Poslovalnica p WHERE z.IDPoslovalnica = p.IDPoslovalnica"
                cursor.execute(sql)
                rows = cursor.fetchall()
                return [
